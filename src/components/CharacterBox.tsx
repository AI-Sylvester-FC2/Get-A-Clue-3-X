  ...
  const handleCheckboxChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    updateSelectedCharacters(prevSelectedCharacters => {
      ...
      if (checked && charToAdd) {
        ...
        if (!alreadyChecked) {
        return [...prevSelectedCharacters, charToAdd];
      }
    } else if (!checked) {
        ...
        {characters.map((character) => (
          <div key={character.id}>
            <label>
              ...
              checked={selectedCharacters.some(char => char.id === character.id)}// determines if the checkbox is checked by look at each char element(object) and finding its id
